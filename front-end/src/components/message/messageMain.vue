      ...
      <mu-list>
        <li style="list-style-type:none;" v-for="items in friendsList">
          <mu-list-item :title=items.from.comName :titleClass="{listFont: true}">
            ...
            <button slot="right" class="contract" v-if="false">与他联系</button>
            <button slot="right" class="mesButton" @click="agree(items._id)">同意</button>
            <button slot="right" class="mesButton" @click="refuse(items._id)">拒绝</button>
...
export default {
  ...
  methods: {
    ...
    async agree (mes) {
      try {
        await this.$http.post('/api/friend/request/' + mes + '/accept');
    ...
    async refuse (mes) {
      try {
        await this.$http.post('/api/friend/request/' + mes + '/refuse');
    ...
    async initData () {
      let res = await this.$http.get('/api/friend/request/receive');
      this.friendsList = res.data;
      if (this.friendsList.length) {
        this.friendsList[0].from.comName += '申请加您为企业好友';
      }
